...
class FeedViewModel: ObservableObject {
    ...
    func requestToJoinProject(postingID: String, completion: @escaping (Bool) -> Void) {
        ...
        userVM?.sendRequest(postingID: postingID) { success in
            if (success) {
                Firestore.firestore().collection("postings").document(postingID).setData(["receivedRequest" : data], merge: true) { error in
                    if let error = error {
